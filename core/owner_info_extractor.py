...
class OwnerInfoExtractor:
    ...
    def extract_from_pdf(
        ...
        if birth_addr_result:
            result.birth_date = birth_addr_result.get("birth_date")
            result.address = birth_addr_result.get("address")
        ...
        if contact_result:
            result.phone = contact_result.get("phone")
        ...
        # 2-4: 인감 분석
        print("\n>>> [2-4단계] 인감 분석...")
        seal_result = self._extract_seal_info(images)
        if seal_result:
            result.has_seal = seal_result.get("has_seal", False)
