...
from core.unified_pdf_analyzer import analyze_pdf_unified
...
# 동시 분석 수: 최대 6개까지 동시에 분석 (업무 시간 단축)
MAX_CONCURRENT_WORKERS = 6
# 슬롯 비었을 때 다음 파일 시작 전 대기(ms). 0이면 즉시 시작
...
class SingleFileWorker(QThread):
    ...
    def run(self):
        try:
            if self.enable_dual:
                # ★ 이중검증 활성화 시 초고속 병렬 분석기 사용
                self.progress.emit(self.file_path, "초고속 병렬 분석 (이중검증)...", 10)
                try:
                    from core.async_parallel_analyzer import analyze_pdf_fast
                    ai_result, meta = analyze_pdf_fast(
                        self.file_path,
                        self.config.announcement_date,
                        provider="gemini",  # Gemini가 더 빠름
                        dual_check=True
                    )
                except ImportError:
                    # 폴백: 기존 분석기 사용
                    self.progress.emit(self.file_path, "PDF 분석 중 (폴백)...", 20)
                    ai_result, meta = analyze_pdf_unified(
                        self.file_path,
                        self.config.announcement_date,
                    )
            else:
                # 일반 모드: 기존 분석기
                self.progress.emit(self.file_path, "PDF 분석 중 (v6.0)...", 20)
                ai_result, meta = analyze_pdf_unified(
                    self.file_path,
                    self.config.announcement_date,
                )
            ...
            self.error.emit(self.file_path, error_detail)
